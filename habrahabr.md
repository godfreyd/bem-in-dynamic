...
Конечно, неудобно перечислять все блоки страницы в нужном порядке. Поэтому обычно в декларации определяют какой-то один блок, который следует рассматривать как центральную «точку входа».

Все остальные БЭМ-сущности попадают в сборку по зависимостям (подробнее читайте про [технологию DEPS](#deps)).
