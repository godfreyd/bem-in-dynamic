...
Например, для того чтобы добавить в сборку блоки `header`, `body` и `footer`, необходимо создать файл `some-block.deps.js` и описать зависимость от этих блоков:

```js
// common.blocks/some-block/some-block.deps.js
({
    block: 'some-block',
    shouldDeps: [
      'header',
