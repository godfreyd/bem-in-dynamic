...
Когда количество блоков переходит рубеж «могу запомнить», возникает проблема их перечисления в нужном порядке. Поэтому обычно в декларации определяют какой-то определенный блок, который следует рассматривать как центральную «точку входа».
