...
Сразу забежим в недалекое будущее — выглядеть оно будет так:


На базе полученной инфраструктуры аналогично можно будет реализовать множество динамических БЭМ-проектов для решения частных задач.

> **Примечание.** Для разработки потребуется установить [следующие модули Node](#Используемые-модули-node).
