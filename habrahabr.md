...
Конечно, перечислять все блоки страницы в нужном порядке неудобно. Поэтому обычно в декларации определяют какой-то один блок, который следует рассматривать как центральную «точку входа». Все остальные БЭМ-сущности попадают в сборку по [зависимостям](#deps).
...
Сборщик соберет проект в следующей последовательности:
...
**Помните:** когда вы описали шаблон с каким-то блоком внутри, вы просто добавли HTML-элемент. Чтобы стили и скрипты этого блока попали в сборку, необходимо описать зависимость от него.
