...
Конечно, неудобно перечислять все блоки страницы в нужном порядке. Поэтому обычно в декларации определяют какой-то один блок, например `root`, который следует рассматривать как центральную «точку входа».
