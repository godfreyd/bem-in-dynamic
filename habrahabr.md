...
**Помните:** когда вы описываете шаблон ([BEMHTML](#bemhtml) или [BEMTREE](#bemtree)) с каким-то блоком внутри, вы просто добавляете новый HTML-элемент. Чтобы стили и скрипты этого блока попали в сборку, необходимо описать зависимость от него.

Например, для того чтобы добавить в сборку блоки `header`, `body` и `footer`, необходимо описать зависимость от этих блоков:
