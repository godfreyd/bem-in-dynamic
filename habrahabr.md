...
Конечно, перечислять все блоки страницы в нужном порядке неудобно. Поэтому обычно в декларации определяют какой-то один блок, который следует рассматривать как центральную «точку входа».

Все остальные БЭМ-сущности попадают в сборку по зависимостям (подробнее в разделе про [технологию DEPS](#deps)).
...
**Правило:** чтобы скрипты и стили нужным вам блоков попали в сборку, необходимо описать зависимость от этих блоков.
