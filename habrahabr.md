...
Методология БЭМ существует достаточно долго и принята на вооружение некоторыми крупными компаниями (Яндекс, Google, epam, BBC, Alfa Bank). При этом она все еще вызывает беспокойство у типичного разработчика и менеджера проектов среднего звена.

У некоторых смельчаков изучение БЭМ не ушло дальше ограничения возможностей CSS для получения более предсказуемых результатов. И хотя БЭМ давно вышел за пределы верстки, до сих пор на вопрос "Знаешь ли ты БЭМ?", можно услышать: "Конечно, это про подчеркивания в классах.".
...
Я считаю, что БЭМ — это лайфхак для стартаперов. По сравнению со многими другими фреймворками и технологиями начало работы с ним требует определенных усилий, но это того стоит. Во все технологии БЭМ (JavaScript, CSS, шаблонизаторы и пр.) изначально заложены принципы хорошей архитектуры. И именно это делает БЭМ таким непонятным на первый взгляд и любимым в итоге.
...
Цель этой статьи заключается в том, чтобы дать читателю представление о базовой функциональности технологий БЭМ. Мы не станем рассматривать вопросы верстки и клиентского JS.
