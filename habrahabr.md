# bem-express — переходим на сторону сервера
...
Сегодня мы расскажем о новом шаблонном репозитории для динамических проектов [bem-express](https://github.com/bem/bem-express), который не только позволяет развернуть БЭМ-проект в один клик, но и делает автоматическую пересборку проекта и перезагрузку браузера. На его основе разработаем динамическое приложение. В рассказе сделаем упор на взаимодействие между собой различных технологий БЭМ. Мы сознательно не будем рассматривать вопросы верстки и клиентского JavaScript, чтобы избежать участи «подчеркивания в классах».
...
10. Нажмите кнопку **Send**.
    ...
    > **Важно!** Сохраните полученные токен и ключи (Consumer Key и Consumer Secret). Они потребуются для [конфигурационного файла](#Конфигурация-приложения) приложения.
...
> **Важно!** Сохраните полученные ключи (Client ID и Client Secret). Они потребуются для [конфигурационного файла](#Конфигурация-приложения) приложения.
