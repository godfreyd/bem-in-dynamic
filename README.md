...
* `index.js` — обвязка над Express. Содержит другие модули, в основном осуществляющеи поддержку промежуточного звена (middleware), которую следует ожидать от приложения с выходом в Интернет.
...
* Создаем пустые `JS`-файлы для будущих модулей:

  * `app.js` — модуль монтирования промежуточных модулей (будет делать их доступными в приложении);
...
* Редактируем файл `index.js`.
  ...
  Меняем весь текущий контент на [следующий](https://gist.github.com/godfreyd/37d903c73f863619e2e1be1cd946d4c3).

  > **Примечание.** Оставляем функциональность отвечающую за запуск приложения и прослушивание запросов на порте.

**Директория `controllers`**

* Создаем пустой `JS`-файл:

  * `index.js` — контроллер обработки запросов и рендеринга HTML.

* Добавляем в `index.js` следующий контент:

  * [Полный код index.js](https://gist.github.com/godfreyd/4bda7da3db029890378e15bcc38f32de).

**Директория `helpers`**

* Создаем пустые `JS`-файлы:
