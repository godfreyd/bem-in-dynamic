...
Как видно из примера, в декларации определен только блок `root`.

При использовании технологии [DEPS](#deps), в декларации достаточно определить одну БЭМ-сущность.

Блок `root` рассматривается как центральная «точка входа» при работе над проектом. Все остальные БЭМ-сущности попадают в сборку по зависимостям.
