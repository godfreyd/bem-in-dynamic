...
4. Получите [Base64-код](#Как-получить-base64-код).
5. Получите [OAuth-токен](#Как-получить-oauth-токен-в-обмен-на-код) (OAuth 2 Bearer-токен) в обмен на Base64-код.
6. Используйте полученный токен и ключи в запросах к Twitter Search API.

> **Примечание.** Postman необходим для получения OAuth 2 Bearer-токена с помощью POST-запроса в обмен на [Base64-код](https://en.wikipedia.org/wiki/Base64). Код генерируется из пары ключей: Consumer Key и Consumer Secret.

### Как получить Base64-код

Чтобы получить Base64-код:
...
4. Введите в поле **Type (or paste) here** строку, полученную на шаге 1.
5. Нажмите кнопку **Encode**.
6. Скопируйте полученный код из поля ниже.

Полученный код необходим, чтобы [получить OAuth-токен](#Как-получить-oauth-токен-в-обмен-на-код).

### Как получить OAuth-токен в обмен на код

Чтобы получить токен в обмен на Base64-код:

1. Запустите Postman.

    > **Примечание.** Окроется вкладка (New Tab), в которой необходимо сформировать запрос к OAuth-серверу Twitter.
...
OAuth-сервер Twitter возвращает токен в [теле ответа](#Формат-ответа-с-токеном).

#### Формат ответа с токеном

Twitter возвращает токен в JSON-формате:
...
Google предлагает приложениям возможность выдавать аутентифицированные запросы от имени самого приложения. Реализация OAuth в Google отличается от реализации в Twitter тем, что Google ограниивает время жизни токена.
