...
* `rebuild.js` — модуль пересборки. Следит за изменениями в файловой структуре проетка (директории: `*.blocks` и `static`), пересобирает и перезапускает проект.
...
> **Примечание.** Middleware или ПО промежуточного звена — это ПО, выполняющее определенные функции, связанные с запросом HTTP. [Список ПО промежуточного звена](http://expressjs.com/en/resources/middleware.html), работающего с Express.
...
В документации Express представлено минимальное приложение «Hello World Express»:
