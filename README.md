...
`*.bemdecl.js`-файлы являются декларациями и определяют список БЭМ-сущностей, используемых на странице:
...
Инструмент сборки, основываясь на данных из декларации, ограничивает количество сущностей, попадающих в готовый проект.

Декларации определяются для каждой страницы в директории `desktop.bundles`:

```files
desktop.bundles/
    */                    # Директория бандлов страницы
        *.bemdecl.js      # Декларация для страницы
```

В локальной копии `bem-express` задекларирован только блок `root`:
...
Это не означает, что на проекте используется один блок. `root` является корневым блоком, а все остальные блоки попадут в сборку по [зависимостям](#depsjs) на основе описанных шаблонов.
...
Зависимости описываются для всех БЭМ-сущностей, которые разнесены по файловой структуре проекта и не отражены в [декларации](#bemdecljs).

Пример зависимостей для блока `root` из локальной копии `bem-express`:
