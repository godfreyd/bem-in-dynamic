...
Согласно протоколу, разработчик регистрирует приложение на OAuth-сервере Twitter или Google и запрашивает доступ к определённым данным. Авторизованный пользователь разрешает или запрещает его.
...
4. Получите [OAuth-токен](#Как-получить-oauth-2-bearer-токен) (OAuth 2 Bearer-токен).
5. Используйте полученный токен и ключи в запросах к Twitter Search API.

> **Примечание.** Postman необходим для получения OAuth 2 Bearer-токена с помощью POST-запроса в обмен на [Base64](https://en.wikipedia.org/wiki/Base64) код. Код генерируется из пары ключей: Consumer Key и Consumer Secret.

### Регистрация приложения

Разработчик приложения должен [зарегистрировать его на OAuth-сервере](https://apps.twitter.com), чтобы получить ключи: Consumer Key, Consumer Secret.

Этапы регистрации:


### Как получить OAuth 2 Bearer-токен
...
Google предлагает приложениям возможность выдавать аутентифицированные запросы от имени самого приложения. Для дополнительной безопасности полученный токен имеет срок жизни. После чего должен быть обновлен.
