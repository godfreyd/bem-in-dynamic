...
4. Получите [OAuth 2 Bearer-токен](#Как-получить-oauth-2-bearer-токен).
5. Используйте полученный токен и ключи в запросах к Twitter Search API.

> **Примечание.** Postman необходим для получения OAuth 2 Bearer-токена с помощью POST-запроса в обмен на [Base64](https://en.wikipedia.org/wiki/Base64) код. Код генерируется из пары Consumer Key, Consumer Secret.
