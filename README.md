...
4. Получите [Base64-код]().
4. Получите [OAuth-токен](#Как-получить-oauth-токен-в-обмен-на-код) (OAuth 2 Bearer-токен) в обмен на [Base64-код](https://en.wikipedia.org/wiki/Base64).
5. Используйте полученный токен и ключи в запросах к Twitter Search API.

> **Примечание.** Postman необходим для получения OAuth 2 Bearer-токена с помощью POST-запроса в обмен на Base64-код. Код генерируется из пары ключей: Consumer Key и Consumer Secret.

### Как получить Base64-код

Чтобы получить Base64-код:

1. Объедените полученные Consumer Key и Consumer Secret в строку вида `Consumer Key:Consumer Secret`.
