...
> **Важно** В документе не будут подробно разбираться вопросы верстки и клиентского JS. Цель документа — показать как разрабатывать динамические проекты по БЭМ, используя полный стек технологий.
...
Генерация запроса и подготовка полученных данных для дальнейшей шаблонизации более подробно рассматриваются в разделе [Работа с API Twitter и YouTube](#).
...
**BEMTREE** является частью шаблонизатора `bem-xjst` и преобзазует BEMJSON с данными в BEMJSON с БЭМ-деревом.

Подробное описание шага рассматривается в разделе "Преобразование данных".

`bem-xjst` содержит два движка:

* **BEMTREE** — преобзазует BEMJSON с данными в BEMJSON с БЭМ-деревом.
