...
Google также предлагает приложениям возможность выдавать аутентифицированные запросы от имени самого приложения. Реализация OAuth в Google отличается от реализации в Twitter тем, что Google ограничивает время жизни токена.
...
2. Зарегистрируйте [приложение](https://developers.google.com/youtube/v3/getting-started) и получите Client ID и Client Secret.
  ...
  > **Важно!** При регистрации или редактировании приложения, необходимо указать callback URL. В нашем случае это http://localhost:3000.
...
* Добавьте в файл `server/config.json` поле `services`, заполнив пустые поля соответствующими значениями:  
