...
Итак, нами упорядочена структура проекта, получены все необходимые [доступы](#Получение-oauth-токенов) для работы с OAuth-серверами Twitter и Google и при запуске приложения (команда `npm run dev`), по адресу [localhost:3000](http://localhost:3000) по-прежнему открывается страница со следующим контентом:
