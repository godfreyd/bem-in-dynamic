...
Содержит конечные точки, которые прослушивают веб-запросы и генерируют страницу.
...
* `rebuild.js` — модуль пересборки. Следит за изменениями в файловой структуре проетка (директории: `*.blocks` и `static`) и пересобирает проект.
