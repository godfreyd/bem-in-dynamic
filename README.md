...
> **Примечание** Также для страницы определяется декларация (файл `index.bemdecl.js`). Декларация представляет собой список блоков, элементов и модификаторов, которые используются на странице. Подробнее о декларациях в БЭМ читайте в разделе [Технологиии](#bemdecljs).
