...
Для этого рассмотрен процесс создания динамического приложения **Social Services Search Robot** (**SSSR**). Оно позволяет искать последние твиты и видео по ключевому слову.

При разработке проекта будем руководствоваться принципами работы с [CSS по БЭМ](https://ru.bem.info/methodology/css/) и использовать:
...
Для этого понадобится локальная копия [шаблонного репозитория bem-express](#Шаблонный-репозиторий). Её можно сделать любым удобным способом:
